 ...
 */
void bqRecorderCallback0(SLAndroidSimpleBufferQueueItf bq, void *rec) {
    (static_cast<AudioRecorder *>(rec))->ProcessSLCallback(bq, 0);
}

void bqRecorderCallback1(SLAndroidSimpleBufferQueueItf bq, void *rec) {
    (static_cast<AudioRecorder *>(rec))->ProcessSLCallback(bq, 1);
}

void bqRecorderCallback2(SLAndroidSimpleBufferQueueItf bq, void *rec) {
    (static_cast<AudioRecorder *>(rec))->ProcessSLCallback(bq, 2);
}

void bqRecorderCallback3(SLAndroidSimpleBufferQueueItf bq, void *rec) {
    (static_cast<AudioRecorder *>(rec))->ProcessSLCallback(bq, 3);
}

void AudioRecorder::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq, int type) {
#ifdef ENABLE_LOG
    recLog_->logTime();
#endif
    ...
    if(type == 0) {ece420ProcessFrame(dataBuf);}
    else if(type == 1) {ece420ProcessFrame_crowd(dataBuf);}
    else if(type == 2) {ece420ProcessFrame_engine(dataBuf);}
    else if(type == 3) {ece420ProcessFrame_helicopter(dataBuf);}
...
AudioRecorder::AudioRecorder(SampleFormat *sampleFormat, SLEngineItf slEngine, int type):
        ...
        callback_(nullptr)
{
    ...
    result = (*recBufQueueItf_)->RegisterCallback(recBufQueueItf_,
                    bqRecorderCallback0, this);
    }
    if(type == 1) {
    result = (*recBufQueueItf_)->RegisterCallback(recBufQueueItf_,
                    bqRecorderCallback1, this);
    }
    if(type == 2) {
    result = (*recBufQueueItf_)->RegisterCallback(recBufQueueItf_,
                    bqRecorderCallback2, this);
    }
    if(type == 3) {
    result = (*recBufQueueItf_)->RegisterCallback(recBufQueueItf_,
                    bqRecorderCallback3, this);
    }
