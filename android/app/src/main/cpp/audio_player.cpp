 ...
 */
void bqPlayerCallback0(SLAndroidSimpleBufferQueueItf bq, void *ctx) {
    (static_cast<AudioPlayer *>(ctx))->ProcessSLCallback(bq, 0);
}

void bqPlayerCallback1(SLAndroidSimpleBufferQueueItf bq, void *ctx) {
    (static_cast<AudioPlayer *>(ctx))->ProcessSLCallback(bq, 1);
}

void bqPlayerCallback2(SLAndroidSimpleBufferQueueItf bq, void *ctx) {
    (static_cast<AudioPlayer *>(ctx))->ProcessSLCallback(bq, 2);
}

void bqPlayerCallback3(SLAndroidSimpleBufferQueueItf bq, void *ctx) {
    (static_cast<AudioPlayer *>(ctx))->ProcessSLCallback(bq, 3);
}

void AudioPlayer::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq, int type) {
#ifdef ENABLE_LOG
    logFile_->logTime();
#endif
...
AudioPlayer::AudioPlayer(SampleFormat *sampleFormat, SLEngineItf slEngine, int type):
    ...
    callback_(nullptr)
{
    ...
    if(type == 0) {
        result = (*playBufferQueueItf_)->RegisterCallback(playBufferQueueItf_, bqPlayerCallback0, this);
    }
    else if(type == 1) {
        result = (*playBufferQueueItf_)->RegisterCallback(playBufferQueueItf_, bqPlayerCallback1, this);
    }
    else if(type == 2) {
        result = (*playBufferQueueItf_)->RegisterCallback(playBufferQueueItf_, bqPlayerCallback2, this);
    }
    else if(type == 3) {
        result = (*playBufferQueueItf_)->RegisterCallback(playBufferQueueItf_, bqPlayerCallback3, this);
    }
    SLASSERT(result);
